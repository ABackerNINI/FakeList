...
class _FakeList_node {
   public:
    ...
    _FakeList_node(const node &_Node)
        : _Size(_Node._Size),
          _Offset(_Node._Offset),
          _Next(_Node._Next),
          _Ptr(_Node._Ptr) {
        if (/*this != &_Node && */ _Ptr) ++_Ptr->_Ref;
    }
    ...
    _Ty &operator[](size_type _Pos) {
        ...
        if (this->_Size < _Pos + 1) throw std::out_of_range("FakeList");
#endif
...
class _FakeList_const_iterator {
   public:
    ...
    size_type operator-(const const_iterator &_Right) const {
        ...
        size_type _Count =
            (_Right._Curnode->_Size - _Right._Curpos) + this->_Curpos;
    ...
    const _Ty *operator->() const { return &((*_Curnode)[_Curpos]); }

    const _Ty &operator*() const { return (*_Curnode)[_Curpos]; }

    bool operator==(const const_iterator &_Right) const {
        return this->_Curpos == _Right._Curpos &&
               this->_Curnode == _Right._Curnode;
    }
    ...
    node *_GetCurnode() const { return _Curnode; }

    size_type _GetCurpos() const { return _Curpos; }

    void _SetCurnode(node *_Curnode) { this->_Curnode = _Curnode; }

    void _SetCurpos(size_type _Curpos) { this->_Curpos = _Curpos; }
...
class _FakeList_iterator : public _FakeList_const_iterator<_Ty> {
   public:
    ...
    _Ty *operator->() const { return &((*base::_Curnode)[base::_Curpos]); }
...
class FakeList {
   public:
    ...
    FakeList &operator=(const FakeList &_Right) { return assign(_Right); }
...
#if (DEBUG & DEBUG_RANGE_CHECK)
        if (_Size < _Pos) throw std::out_of_range("FakeList");
#endif
    ...
    FakeList &push_front(const _Ty &_Elem) { return push_front(&_Elem, 1); }
    ...
    FakeList &push_back(const _Ty &_Elem) { return push_back(&_Elem, 1); }

    FakeList &push_back(_Ty &&_Elem) { return push_back(std::move(&_Elem), 1); }
    ...
    */
...
#if (DEBUG & DEBUG_RANGE_CHECK)
        if (_Begin + _Count > _Size) throw std::out_of_range("FakeList");
#endif
        node *_L, *_R;
    ...
    FakeList &erase(const_iterator _Begin, const_iterator _End) {
        ...
        if (_Begin._GetCurnode() == _End._GetCurnode()) {
            ...
            if (_Begin._GetCurpos() == 0) {
                ...
                // not from beginning,separate the node,and disable _End
                // iterator
    ...
    FakeList &replace(const_iterator _Begin, const_iterator _End,
                      const _Ty *_Newval, int _Newval_Len) {
        size_type _Pos =
            _PosList(_End._GetCurnode(), _End._GetCurpos()) - (_End - _Begin);
    ...
    FakeList &replace(size_type _Begin, size_type _Count, const _Ty *_Newval,
                      size_type _Newval_Len) {
        node *_L, *_R;
    ...
    iterator begin() NOEXCEPT { return iterator(_Front, 0); }
    ...
    FakeList clone(
        size_type _Max_size_of_each_node = DEFAULT_SIZE_OF_EACH_NODE) const {
        FakeList _Ret;

        if (this->_Front) {
            size_type _Num =
                this->_Size / _Max_size_of_each_node +
                (this->_Size % _Max_size_of_each_node ? 1 : 0);  // nodes needed
            size_type _Len;
            size_type _Rest = this->_Size;

            _Len = (_Rest >= _Max_size_of_each_node ? _Max_size_of_each_node
                                                    : _Rest);
            ...
            for (const_iterator _Src = this->begin(); _Src != this->end();
                 ++_Src, ++_Des) {
                *_Des = *_Src;
            }
...
class string_builder : public FakeList<char> {
   public:
    ...
    string_builder(const char *_String) : base(_String, strlen(_String)) {}
    ...
    string_builder(const string_builder &_Str_builder) : base(_Str_builder) {}
    ...
    string_builder clone(size_type _Max_size_of_each_node =
                             DEFAULT_SIZE_OF_EACH_NODE_CHAR) const {
        string_builder _Ret;
    ...
    string_builder &format(
        size_type _Max_size_of_each_node = DEFAULT_SIZE_OF_EACH_NODE_CHAR) {
    ...
    void print(bool _Add_LF = false) const {
        ...
        for (const_iterator _Iter = this->begin(); _Iter != this->end();
             ++_Iter)
            printf("%c", *_Iter);
...
inline string_builder operator+(const char *_String,
                                string_builder &&_String_builder) {
